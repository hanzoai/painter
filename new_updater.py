...
def update_windows_updater():
    ...
    bat_path = os.path.join(base_path, ".ci/update_windows/update_studioui.bat")

    dest_updater_path = os.path.join(top_path, "update/update.py")
    dest_bat_path = os.path.join(top_path, "update/update_studioui.bat")
    dest_bat_deps_path = os.path.join(top_path, "update/update_studioui_and_python_dependencies.bat")
    ...
    try:
        with open(dest_bat_deps_path, 'rb') as f:
            contents = f.read()
            contents = contents.replace(b'..\\python_embeded\\python.exe .\\update.py ..\\Studio\\', b'call update_studioui.bat nopause')
