...
from studio.samplers import KSampler
...
DEFAULT_STUDIO_GRAPH = StudioGraph(graph=default_graph, sampler_nodes=['10','14'])
DEFAULT_STUDIO_GRAPH_ID = os.path.splitext(os.path.basename(default_graph_file))[0]

#
# Loop through these variables
#
studio_graph_list = [DEFAULT_STUDIO_GRAPH]
studio_graph_ids = [DEFAULT_STUDIO_GRAPH_ID]
prompt_list = [
    'a painting of a cat',
]
...
class TestInference:
    ...
    @fixture(scope="class", params=studio_graph_list, ids=studio_graph_ids, autouse=True)
    def _client_graph(self, request, args_pytest, _server) -> (StudioClient, StudioGraph):
        studio_graph = request.param

        # Start client
        studio_client = self.start_client(args_pytest["listen"], args_pytest["port"])

        # Warm up pipeline
        studio_client.get_images(graph=studio_graph.graph, save=False)

        yield studio_client, studio_graph
        del studio_client
        del studio_graph
        torch.cuda.empty_cache()
    ...
    @fixture
    def studio_graph(self, _client_graph):
    ...
    def test_studio(
        ...
        studio_graph.set_filename_prefix(test_info)
        # Settings for studio graph
        studio_graph.set_sampler_name(sampler)
        studio_graph.set_scheduler(scheduler)
        studio_graph.set_prompt(prompt)

        # Generate
        images = client.get_images(studio_graph.graph)
