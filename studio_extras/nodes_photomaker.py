...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim, use_residual=True, operations=studio.ops):
        super().__init__()
        if use_residual:
...
class PhotoMakerIDEncoder(studio.clip_model.CLIPVisionModelProjection):
    def __init__(self):
        ...
        dtype = studio.model_management.text_encoder_dtype(self.load_device)

        super().__init__(VISION_CONFIG_DICT, dtype, offload_device, studio.ops.manual_cast)
        self.visual_projection_2 = studio.ops.manual_cast.Linear(1024, 1280, bias=False)
        self.fuse_module = FuseModule(2048, studio.ops.manual_cast)
...
class PhotoMakerLoader(io.StudioNode):
    @classmethod
    ...
    def execute(cls, photomaker_model_name):
        ...
        data = studio.utils.load_torch_file(photomaker_model_path, safe_load=True)
...
class PhotoMakerEncode(io.StudioNode):
    @classmethod
    def define_schema(cls):
        return io.Schema(
    ...
    def execute(cls, photomaker, image, clip, text):
        ...
        pixel_values = studio.clip_vision.clip_preprocess(image.to(photomaker.load_device)).float()
        try:
...
class PhotomakerExtension(StudioExtension):
    @override
...
async def studio_entrypoint() -> PhotomakerExtension:
