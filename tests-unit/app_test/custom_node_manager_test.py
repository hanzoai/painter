...
def app(custom_node_manager):
    ...
    custom_node_manager.add_routes(
        routes, app, [("Studio-TestExtension1", "Studio-TestExtension1")]
    )
...
async def test_get_workflow_templates(aiohttp_client, app, tmp_path):
    ...
    example_workflows_dir = (
        custom_nodes_dir / "Studio-TestExtension1" / "example_workflows"
    )
    ...
    with patch(
        ...
        assert "Studio-TestExtension1" in workflows_dict
        assert isinstance(workflows_dict["Studio-TestExtension1"], list)
        assert workflows_dict["Studio-TestExtension1"][0] == "workflow1"
