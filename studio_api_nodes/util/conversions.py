...
from studio.utils import common_upscale
from studio_api.latest import Input, InputImpl
...
def audio_tensor_to_contiguous_ndarray(waveform: torch.Tensor) -> np.ndarray:
    ...
    Args:
        waveform: a tensor of shape (1, channels, samples) derived from a Studio `AUDIO` type.
...
def audio_bytes_to_audio_input(audio_bytes: bytes) -> dict:
    ...
    a Studio AUDIO dict: {"waveform": [1, C, T] float32, "sample_rate": int}.
    """
