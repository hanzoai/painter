...
from studio_api.latest import IO, Input
from studio_api.util import VideoCodec, VideoContainer
...
async def upload_images_to_studioapi(
    ...
    *,
    ...
    """
    Uploads images to Studio API and returns download URLs.
    ...
    for idx in range(min(batch_len, max_images)):
        ...
        url = await upload_file_to_studioapi(cls, img_io, img_io.name, mime_type, wait_label)
        download_urls.append(url)
    return download_urls


async def upload_audio_to_studioapi(
    cls: type[IO.StudioNode],
    audio: Input.Audio,
    *,
    ...
    """
    Uploads a single audio input to Studio API and returns its download URL.
    ...
    return await upload_file_to_studioapi(cls, audio_bytes_io, filename, mime_type)


async def upload_video_to_studioapi(
    cls: type[IO.StudioNode],
    video: Input.Video,
    *,
    ...
    """
    Uploads a single video to Studio API and returns its download URL.
    ...
    return await upload_file_to_studioapi(cls, video_bytes_io, filename, upload_mime_type)


async def upload_file_to_studioapi(
    cls: type[IO.StudioNode],
    ...
    """Uploads a single file to Studio API and returns its download URL."""
...
async def upload_file(
    ...
    *,
    ...
    """
    Upload a file to a signed URL (e.g., S3 pre-signed PUT) with retries, Studio progress display, and interruption.
    ...
    Args:
        ...
        wait_label: Progress label shown in Studio UI.
