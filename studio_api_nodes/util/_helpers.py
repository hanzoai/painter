...
from studio.model_management import processing_interrupted
...
def get_node_id(node_cls: type[IO.StudioNode]) -> str:
    return node_cls.hidden.unique_id


def get_auth_header(node_cls: type[IO.StudioNode]) -> dict[str, str]:
    if node_cls.hidden.auth_token_studio_org:
        return {"Authorization": f"Bearer {node_cls.hidden.auth_token_studio_org}"}
    if node_cls.hidden.api_key_studio_org:
        return {"X-API-KEY": node_cls.hidden.api_key_studio_org}
    return {}
...
async def sleep_with_interrupt(
    seconds: float,
    node_cls: Optional[type[IO.StudioNode]],
    label: Optional[str] = None,
    start_ts: Optional[float] = None,
    estimated_total: Optional[int] = None,
    *,
    display_callback: Optional[Callable[[type[IO.StudioNode], str, int, Optional[int]], None]] = None,
):
    """
