...
class LoKrAdapter(WeightAdapterBase):
    ...
    def calculate_weight(
        ...
        if w1 is None:
            dim = w1_b.shape[0]
            w1 = torch.mm(studio.model_management.cast_to_device(w1_a, weight.device, intermediate_dtype),
                            studio.model_management.cast_to_device(w1_b, weight.device, intermediate_dtype))
        else:
            w1 = studio.model_management.cast_to_device(w1, weight.device, intermediate_dtype)

        if w2 is None:
            dim = w2_b.shape[0]
            if t2 is None:
                w2 = torch.mm(studio.model_management.cast_to_device(w2_a, weight.device, intermediate_dtype),
                                studio.model_management.cast_to_device(w2_b, weight.device, intermediate_dtype))
            else:
                w2 = torch.einsum('i j k l, j r, i p -> p r k l',
                                    studio.model_management.cast_to_device(t2, weight.device, intermediate_dtype),
                                    studio.model_management.cast_to_device(w2_b, weight.device, intermediate_dtype),
                                    studio.model_management.cast_to_device(w2_a, weight.device, intermediate_dtype))
        else:
            w2 = studio.model_management.cast_to_device(w2, weight.device, intermediate_dtype)
