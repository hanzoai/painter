    ...
    def calculate_weight(
        ...
        if v[5] is not None: #cp decomposition
            ...
            m1 = torch.einsum('i j k l, j r, i p -> p r k l',
                                studio.model_management.cast_to_device(t1, weight.device, intermediate_dtype),
                                studio.model_management.cast_to_device(w1b, weight.device, intermediate_dtype),
                                studio.model_management.cast_to_device(w1a, weight.device, intermediate_dtype))

            m2 = torch.einsum('i j k l, j r, i p -> p r k l',
                                studio.model_management.cast_to_device(t2, weight.device, intermediate_dtype),
                                studio.model_management.cast_to_device(w2b, weight.device, intermediate_dtype),
                                studio.model_management.cast_to_device(w2a, weight.device, intermediate_dtype))
        else:
            m1 = torch.mm(studio.model_management.cast_to_device(w1a, weight.device, intermediate_dtype),
                            studio.model_management.cast_to_device(w1b, weight.device, intermediate_dtype))
            m2 = torch.mm(studio.model_management.cast_to_device(w2a, weight.device, intermediate_dtype),
                            studio.model_management.cast_to_device(w2b, weight.device, intermediate_dtype))
