...
class VAE(nn.Module):
    ...
    def normalize(self, x: torch.Tensor) -> torch.Tensor:
        return (x - studio.model_management.cast_to(self.data_mean, dtype=x.dtype, device=x.device)) / studio.model_management.cast_to(self.data_std, dtype=x.dtype, device=x.device)

    def unnormalize(self, x: torch.Tensor) -> torch.Tensor:
        return x * studio.model_management.cast_to(self.data_std, dtype=x.dtype, device=x.device) + studio.model_management.cast_to(self.data_mean, dtype=x.dtype, device=x.device)
