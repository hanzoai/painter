...
from studio.ldm.lightricks.model import TimestepEmbedding, Timesteps
import torch.nn.functional as F

from studio.ldm.flux.math import apply_rope, rope
from studio.ldm.flux.layers import LastLayer
...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        logits = F.linear(hidden_states, studio.model_management.cast_to(self.weight, dtype=hidden_states.dtype, device=hidden_states.device), None)
...
class HiDreamImageTransformer2DModel(nn.Module):
    ...
    def _forward(
        ...
        hidden_states = studio.ldm.common_dit.pad_to_patch_size(x, (self.patch_size, self.patch_size))
        timesteps = t
        pooled_embeds = y
