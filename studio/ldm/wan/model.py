...
from studio.ldm.modules.attention import optimized_attention
from studio.ldm.flux.layers import EmbedND
from studio.ldm.flux.math import apply_rope1
import studio.ldm.common_dit
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if e.ndim < 4:
            e = (studio.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device) + e).chunk(6, dim=1)
        else:
            e = (studio.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device).unsqueeze(0) + e).unbind(2)
...
class Head(nn.Module):
    ...
    def forward(self, x, e):
        ...
        # assert e.dtype == torch.float32
        if e.ndim < 3:
            e = (studio.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device) + e.unsqueeze(1)).chunk(2, dim=1)
        else:
            e = (studio.model_management.cast_to(self.modulation, dtype=x.dtype, device=x.device).unsqueeze(0) + e.unsqueeze(2)).unbind(2)
...
class MLPProj(torch.nn.Module):
    ...
    def forward(self, image_embeds):
        if self.emb_pos is not None:
            image_embeds = image_embeds[:, :self.emb_pos.shape[1]] + studio.model_management.cast_to(self.emb_pos[:, :image_embeds.shape[1]], dtype=image_embeds.dtype, device=image_embeds.device)
...
class WanModel(torch.nn.Module):
    ...
    def forward(self, x, timestep, context, clip_fea=None, time_dim_concat=None, transformer_options={}, **kwargs):
        return studio.patcher_extension.WrapperExecutor.new_class_executor(
            self._forward,
            self,
            studio.patcher_extension.get_all_wrappers(studio.patcher_extension.WrappersMP.DIFFUSION_MODEL, transformer_options)
    ...
    def _forward(self, x, timestep, context, clip_fea=None, time_dim_concat=None, transformer_options={}, **kwargs):
        bs, c, t, h, w = x.shape
        x = studio.ldm.common_dit.pad_to_patch_size(x, self.patch_size)

        t_len = t
        if time_dim_concat is not None:
            time_dim_concat = studio.ldm.common_dit.pad_to_patch_size(time_dim_concat, self.patch_size)
...
class MotionEncoder_tc(nn.Module):
    ...
    def forward(self, x):
        ...
        padding = studio.model_management.cast_to(self.padding_tokens, dtype=x.dtype, device=x.device).repeat(b, x.shape[1], 1, 1)
...
class CausalAudioEncoder(nn.Module):
    ...
    def forward(self, features):
        ...
        weights = self.act(studio.model_management.cast_to(self.weights, dtype=features.dtype, device=features.device))
...
class WanModel_S2V(WanModel):
    ...
    def forward_orig(
        ...
        **kwargs,
        ...
        cond_mask_weight = studio.model_management.cast_to(self.trainable_cond_mask.weight, dtype=x.dtype, device=x.device).unsqueeze(1).unsqueeze(1)
