...
class FusedLeakyReLU(torch.nn.Module):
    ...
    def forward(self, input):
        return fused_leaky_relu(input, studio.model_management.cast_to(self.bias, device=input.device, dtype=input.dtype), self.negative_slope, self.scale)
...
class Blur(torch.nn.Module):
    ...
    def forward(self, input):
        return upfirdn2d(input, studio.model_management.cast_to(self.kernel, dtype=input.dtype, device=input.device), pad=self.pad)
...
class EqualConv2d(torch.nn.Module):
    ...
    def forward(self, input):
        if self.bias is None:
            ...
            bias = studio.model_management.cast_to(self.bias, device=input.device, dtype=input.dtype)

        return F.conv2d(input, studio.model_management.cast_to(self.weight, device=input.device, dtype=input.dtype) * self.scale, bias=bias, stride=self.stride, padding=self.padding)
...
class EqualLinear(torch.nn.Module):
    ...
    def forward(self, input):
        if self.bias is None:
            ...
            bias = studio.model_management.cast_to(self.bias, device=input.device, dtype=input.dtype) * self.lr_mul

        if self.activation:
            out = F.linear(input, studio.model_management.cast_to(self.weight, device=input.device, dtype=input.dtype) * self.scale)
            return fused_leaky_relu(out, bias)
        return F.linear(input, studio.model_management.cast_to(self.weight, device=input.device, dtype=input.dtype) * self.scale, bias=bias)
...
class Direction(torch.nn.Module):
    ...
    def forward(self, input):
        stabilized_weight = studio.model_management.cast_to(self.weight, device=input.device, dtype=input.dtype) + 1e-8 * torch.eye(512, self.motion_dim, device=input.device, dtype=input.dtype)
