...
class HiDreamTEModel(torch.nn.Module):
    def __init__(self, clip_l=True, clip_g=True, t5=True, llama=True, dtype_t5=None, dtype_llama=None, device="cpu", dtype=None, model_options={}):
        ...
        if llama:
            dtype_llama = studio.model_management.pick_weight_dtype(dtype_llama, dtype, device)
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if t5_out is None:
            t5_out = torch.zeros((1, 128, 4096), device=studio.model_management.intermediate_device())

        if ll_out is None:
            ll_out = torch.zeros((1, 32, 1, 4096), device=studio.model_management.intermediate_device())
