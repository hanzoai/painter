...
class ResourcesLocal(Resources):
    ...
    def get(self, key: ResourceKey, default: Any=...) -> Any:
        ...
        if isinstance(key, TorchDictFolderFilename):
            if default is ...:
                to_return = studio.utils.load_torch_file(folder_paths.get_full_path_or_raise(key.folder_name, key.file_name), safe_load=True)
            else:
                full_path = folder_paths.get_full_path(key.folder_name, key.file_name)
                if full_path is not None:
                    to_return = studio.utils.load_torch_file(full_path, safe_load=True)
