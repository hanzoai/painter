...
if TYPE_CHECKING:
    ...
    from studio.clip_vision import ClipVisionModel
    from studio.clip_vision import Output as ClipVisionOutput_
    from studio.controlnet import ControlNet
    from studio.hooks import HookGroup, HookKeyframeGroup
    from studio.model_patcher import ModelPatcher
    from studio.samplers import CFGGuider, Sampler
    from studio.sd import CLIP, VAE
    from studio.sd import StyleModel as StyleModel_
    from studio_api.input import VideoInput
from studio_api.internal import (_StudioNodeInternal, _NodeOutputInternal, classproperty, copy_class, first_real_override, is_class,
    prune_dict, shallow_clone_class)
from studio_api.latest._resources import Resources, ResourcesLocal
from studio_execution.graph_utils import ExecutionBlocker
from ._util import MESH, VOXEL

# from studio_extras.nodes_images import SVG as SVG_ # NOTE: needs to be moved before can be imported due to circular reference
...
class _StudioType(ABC):
    Type = Any
    io_type: str = None

# NOTE: this is a workaround to make the decorator return the correct type
T = TypeVar("T", bound=type)
def studiotype(io_type: str, **kwargs):
    '''
    Decorator to mark nested classes as StudioType; io_type will be bound to the class.

    A StudioType may have the following attributes:
    - Type = <type hint here>
    - class Input(Input): ...
    - class Output(Output): ...
    '''
    def decorator(cls: T) -> T:
        if isinstance(cls, _StudioType) or issubclass(cls, _StudioType):
            ...
            # assign StudioType attributes, if needed
...
def Custom(io_type: str) -> type[StudioTypeIO]:
    '''Create a StudioType for a custom io_type.'''
    @studiotype(io_type=io_type)
    class CustomStudioType(StudioTypeIO):
        ...
    return CustomStudioType

class _IO_V3:
    '''
    Base class for V3 Inputs and Outputs.
    '''
    Parent: _StudioType = None
...
class StudioTypeI(_StudioType):
    '''StudioType subclass that only has a default Input class - intended for types that only have Inputs.'''
    class Input(Input):
        ...

class StudioTypeIO(StudioTypeI):
    '''StudioType subclass that has default Input and Output classes; useful for types with both Inputs and Outputs.'''
    class Output(Output):
        ...


@studiotype(io_type="BOOLEAN")
class Boolean(StudioTypeIO):
    Type = bool
...
@studiotype(io_type="INT")
class Int(StudioTypeIO):
    Type = int
...
@studiotype(io_type="FLOAT")
class Float(StudioTypeIO):
    Type = float
...
@studiotype(io_type="STRING")
class String(StudioTypeIO):
    Type = str
...
@studiotype(io_type="COMBO")
class Combo(StudioTypeIO):
    Type = str
    class Input(WidgetInput):
...
@studiotype(io_type="IMAGE")
class Image(StudioTypeIO):
    Type = torch.Tensor


@studiotype(io_type="WAN_CAMERA_EMBEDDING")
class WanCameraEmbedding(StudioTypeIO):
    Type = torch.Tensor


@studiotype(io_type="WEBCAM")
class Webcam(StudioTypeIO):
    Type = str
...
@studiotype(io_type="MASK")
class Mask(StudioTypeIO):
    Type = torch.Tensor

@studiotype(io_type="LATENT")
class Latent(StudioTypeIO):
...
@studiotype(io_type="CONDITIONING")
class Conditioning(StudioTypeIO):
...
@studiotype(io_type="SAMPLER")
class Sampler(StudioTypeIO):
    if TYPE_CHECKING:
        Type = Sampler

@studiotype(io_type="SIGMAS")
class Sigmas(StudioTypeIO):
    Type = torch.Tensor

@studiotype(io_type="NOISE")
class Noise(StudioTypeIO):
    Type = torch.Tensor

@studiotype(io_type="GUIDER")
class Guider(StudioTypeIO):
    if TYPE_CHECKING:
        Type = CFGGuider

@studiotype(io_type="CLIP")
class Clip(StudioTypeIO):
    if TYPE_CHECKING:
        Type = CLIP

@studiotype(io_type="CONTROL_NET")
class ControlNet(StudioTypeIO):
    if TYPE_CHECKING:
        Type = ControlNet

@studiotype(io_type="VAE")
class Vae(StudioTypeIO):
    if TYPE_CHECKING:
        Type = VAE

@studiotype(io_type="MODEL")
class Model(StudioTypeIO):
    if TYPE_CHECKING:
        Type = ModelPatcher

@studiotype(io_type="CLIP_VISION")
class ClipVision(StudioTypeIO):
    if TYPE_CHECKING:
        Type = ClipVisionModel

@studiotype(io_type="CLIP_VISION_OUTPUT")
class ClipVisionOutput(StudioTypeIO):
    if TYPE_CHECKING:
        Type = ClipVisionOutput_

@studiotype(io_type="STYLE_MODEL")
class StyleModel(StudioTypeIO):
    if TYPE_CHECKING:
        Type = StyleModel_

@studiotype(io_type="GLIGEN")
class Gligen(StudioTypeIO):
    '''ModelPatcher that wraps around a 'Gligen' model.'''
    if TYPE_CHECKING:
        Type = ModelPatcher

@studiotype(io_type="UPSCALE_MODEL")
class UpscaleModel(StudioTypeIO):
    if TYPE_CHECKING:
        Type = ImageModelDescriptor

@studiotype(io_type="LATENT_UPSCALE_MODEL")
class LatentUpscaleModel(StudioTypeIO):
    Type = Any

@studiotype(io_type="AUDIO")
class Audio(StudioTypeIO):
    class AudioDict(TypedDict):
        waveform: torch.Tensor
        sampler_rate: int
    Type = AudioDict

@studiotype(io_type="VIDEO")
class Video(StudioTypeIO):
    if TYPE_CHECKING:
        Type = VideoInput

@studiotype(io_type="SVG")
class SVG(StudioTypeIO):
    Type = Any # TODO: SVG class is defined in studio_extras/nodes_images.py, causing circular reference; should be moved to somewhere else before referenced directly in v3

@studiotype(io_type="LORA_MODEL")
class LoraModel(StudioTypeIO):
    Type = dict[str, torch.Tensor]

@studiotype(io_type="LOSS_MAP")
class LossMap(StudioTypeIO):
    class LossMapDict(TypedDict):
        loss: list[torch.Tensor]
    Type = LossMapDict

@studiotype(io_type="VOXEL")
class Voxel(StudioTypeIO):
    Type = VOXEL

@studiotype(io_type="MESH")
class Mesh(StudioTypeIO):
    Type = MESH

@studiotype(io_type="HOOKS")
class Hooks(StudioTypeIO):
    if TYPE_CHECKING:
        Type = HookGroup

@studiotype(io_type="HOOK_KEYFRAMES")
class HookKeyframes(StudioTypeIO):
    if TYPE_CHECKING:
        Type = HookKeyframeGroup

@studiotype(io_type="TIMESTEPS_RANGE")
class TimestepsRange(StudioTypeIO):
    '''Range defined by start and endpoint, between 0.0 and 1.0.'''
    Type = tuple[int, int]

@studiotype(io_type="LATENT_OPERATION")
class LatentOperation(StudioTypeIO):
    Type = Callable[[torch.Tensor], torch.Tensor]

@studiotype(io_type="FLOW_CONTROL")
class FlowControl(StudioTypeIO):
    # NOTE: only used in testing_nodes right now
    Type = tuple[str, Any]

@studiotype(io_type="ACCUMULATION")
class Accumulation(StudioTypeIO):
    # NOTE: only used in testing_nodes right now
...
@studiotype(io_type="LOAD3D_CAMERA")
class Load3DCamera(StudioTypeIO):
...
@studiotype(io_type="LOAD_3D")
class Load3D(StudioTypeIO):
...
@studiotype(io_type="LOAD_3D_ANIMATION")
class Load3DAnimation(Load3D):
    ...


@studiotype(io_type="PHOTOMAKER")
class Photomaker(StudioTypeIO):
    Type = Any


@studiotype(io_type="POINT")
class Point(StudioTypeIO):
    Type = Any # NOTE: I couldn't find any references in core code to POINT io_type. Does this exist?

@studiotype(io_type="FACE_ANALYSIS")
class FaceAnalysis(StudioTypeIO):
    Type = Any # NOTE: I couldn't find any references in core code to POINT io_type. Does this exist?

@studiotype(io_type="BBOX")
class BBOX(StudioTypeIO):
    Type = Any # NOTE: I couldn't find any references in core code to POINT io_type. Does this exist?

@studiotype(io_type="SEGS")
class SEGS(StudioTypeIO):
    Type = Any # NOTE: I couldn't find any references in core code to POINT io_type. Does this exist?

@studiotype(io_type="*")
class AnyType(StudioTypeIO):
    Type = Any

@studiotype(io_type="MODEL_PATCH")
class MODEL_PATCH(StudioTypeIO):
    Type = Any

@studiotype(io_type="AUDIO_ENCODER")
class AudioEncoder(StudioTypeIO):
    Type = Any

@studiotype(io_type="AUDIO_ENCODER_OUTPUT")
class AudioEncoderOutput(StudioTypeIO):
    Type = Any

@studiotype(io_type="STUDIO_MULTITYPED_V3")
class MultiType:
    Type = Any
    class Input(Input):
        '''
        Input that permits more than one input type; if `id` is an instance of `StudioType.Input`, then that input will be used to create a widget (if applicable) with overridden values.
        '''
        def __init__(self, id: str | Input, types: list[type[_StudioType] | _StudioType], display_name: str=None, optional=False, tooltip: str=None, lazy: bool=None, extra_dict=None):
...
@studiotype(io_type="STUDIO_AUTOGROW_V3")
class AutogrowDynamic(StudioTypeI):
    Type = list[Any]
...
@studiotype(io_type="STUDIO_COMBODYNAMIC_V3")
class ComboDynamic(StudioTypeI):
    class Input(DynamicInput):
        def __init__(self, id: str):
            pass

@studiotype(io_type="STUDIO_MATCHTYPE_V3")
class MatchType(StudioTypeIO):
    class Template:
        def __init__(self, template_id: str, allowed_types: _StudioType | list[_StudioType]):
            self.template_id = template_id
            self.allowed_types = [allowed_types] if isinstance(allowed_types, _StudioType) else allowed_types
...
class HiddenHolder:
    def __init__(self, unique_id: str, prompt: Any,
                 ...
                 auth_token_studio_org: str, api_key_studio_org: str, **kwargs):
        ...
        """DYNPROMPT is an instance of studio_execution.graph.DynamicPrompt. It differs from PROMPT in that it may mutate during the course of execution in response to Node Expansion."""
        self.auth_token_studio_org = auth_token_studio_org
        """AUTH_TOKEN_STUDIO_ORG is a token acquired from signing into a StudioOrg account on frontend."""
        self.api_key_studio_org = api_key_studio_org
        """API_KEY_STUDIO_ORG is an API Key generated by StudioOrg that allows skipping signing into a StudioOrg account on frontend."""
    ...
    def from_dict(cls, d: dict | None):
        ...
        return cls(
            ...
            auth_token_studio_org=d.get(Hidden.auth_token_studio_org, None),
            api_key_studio_org=d.get(Hidden.api_key_studio_org, None),
        )
...
class Hidden(str, Enum):
    ...
    dynprompt = "DYNPROMPT"
    """DYNPROMPT is an instance of studio_execution.graph.DynamicPrompt. It differs from PROMPT in that it may mutate during the course of execution in response to Node Expansion."""
    auth_token_studio_org = "AUTH_TOKEN_STUDIO_ORG"
    """AUTH_TOKEN_STUDIO_ORG is a token acquired from signing into a StudioOrg account on frontend."""
    api_key_studio_org = "API_KEY_STUDIO_ORG"
    """API_KEY_STUDIO_ORG is an API Key generated by StudioOrg that allows skipping signing into a StudioOrg account on frontend."""
    ...
    def finalize(self):
        ...
        if self.is_api_node:
            if self.hidden is None:
                self.hidden = []
            if Hidden.auth_token_studio_org not in self.hidden:
                self.hidden.append(Hidden.auth_token_studio_org)
            if Hidden.api_key_studio_org not in self.hidden:
                self.hidden.append(Hidden.api_key_studio_org)
...
class _StudioNodeBaseInternal(_StudioNodeInternal):
    ...
    def GET_BASE_CLASS(cls):
        return _StudioNodeBaseInternal
    ...
    @final
    @classmethod
    def PREPARE_CLASS_CLONE(cls, hidden_inputs: dict) -> type[StudioNode]:
        """Creates clone of real node class to prevent monkey-patching."""
        c_type: type[StudioNode] = cls if is_class(cls) else type(cls)
        type_clone: type[StudioNode] = shallow_clone_class(c_type)
        # set hidden
...
class StudioNode(_StudioNodeBaseInternal):
