...
from studio_api.latest._io import StudioNode, FolderType, Image, _UIOutput
...
class ImageSaveHelper:
    ...
    @staticmethod
    def _create_png_metadata(cls: Type[StudioNode] | None) -> PngInfo | None:
    ...
    @staticmethod
    def _create_animated_png_metadata(cls: Type[StudioNode] | None) -> PngInfo | None:
    ...
    @staticmethod
    def _create_webp_metadata(pil_image: PILImage.Image, cls: Type[StudioNode] | None) -> PILImage.Exif:
    ...
    @staticmethod
    def save_images(
        images, filename_prefix: str, folder_type: FolderType, cls: Type[StudioNode] | None, compress_level = 4,
    ) -> list[SavedResult]:
    ...
    @staticmethod
    def get_save_images_ui(images, filename_prefix: str, cls: Type[StudioNode] | None, compress_level=4) -> SavedImages:
    ...
    def save_animated_png(
        images, filename_prefix: str, folder_type: FolderType, cls: Type[StudioNode] | None, fps: float, compress_level: int
    ) -> SavedResult:
    ...
    def get_save_animated_png_ui(
        images, filename_prefix: str, cls: Type[StudioNode] | None, fps: float, compress_level: int
    ) -> SavedImages:
...
class AudioSaveHelper:
    ...
    def get_save_audio_ui(
        audio, filename_prefix: str, cls: Type[StudioNode] | None, format: str = "flac", quality: str = "128k",
    ) -> SavedAudios:
...
class PreviewImage(_UIOutput):
    def __init__(self, image: Image.Type, animated: bool = False, cls: Type[StudioNode] = None, **kwargs):
        self.values = ImageSaveHelper.save_images(
            image,
            filename_prefix="Studio_temp_" + ''.join(random.choice("abcdefghijklmnopqrstupvxyz") for _ in range(5)),
...
class PreviewMask(PreviewImage):
    def __init__(self, mask: PreviewMask.Type, animated: bool=False, cls: StudioNode=None, **kwargs):
        preview = mask.reshape((-1, 1, mask.shape[-2], mask.shape[-1])).movedim(1, -1).expand(-1, -1, -1, 3)
        super().__init__(preview, animated, cls, **kwargs)


class PreviewAudio(_UIOutput):
    def __init__(self, audio: dict, cls: Type[StudioNode] = None, **kwargs):
        self.values = AudioSaveHelper.save_audio(
            audio,
            filename_prefix="Studio_temp_" + "".join(random.choice("abcdefghijklmnopqrstuvwxyz") for _ in range(5)),
            folder_type=FolderType.temp,
            cls=cls,
            format="flac",
            quality="128k",
