# Internal infrastructure for StudioAPI
from .api_registry import (
    StudioAPIBase as StudioAPIBase,
    StudioAPIWithVersion as StudioAPIWithVersion,
...
def first_real_override(cls: type, name: str, *, base: type=None) -> Optional[Callable]:
    ...
    if base is None:
        if not hasattr(cls, "GET_BASE_CLASS"):
            raise ValueError("base is required if cls does not have a GET_BASE_CLASS; is this a valid StudioNode subclass?")
        base = cls.GET_BASE_CLASS()
    base_attr = getattr(base, name, None)
    if base_attr is None:
        return None
    base_func = base_attr.__func__
    for c in cls.mro():                       # NodeB, NodeA, StudioNode, object …
...
class _StudioNodeInternal:
    """Class that all V3-based APIs inherit from for StudioNode.
